...
#define PIN_RFM96_RST            GPIO_NUM_14
#define PIN_RFM96_DIO_0          GPIO_NUM_27
...
lora_write_reg(int reg, int val)
{
   ...
   gpio_set_level(PIN_RFM96_SPI_CS, 0);
   spi_device_transmit(__spi, &t);
   gpio_set_level(PIN_RFM96_SPI_CS, 1);
}
...
lora_reset(void)
{
   gpio_set_level(PIN_RFM96_RST, 0);
   vTaskDelay(pdMS_TO_TICKS(1));
   gpio_set_level(PIN_RFM96_RST, 1);
...
lora_init(void)
{
    ...
    */
   gpio_pad_select_gpio(PIN_RFM96_RST);
   gpio_set_direction(PIN_RFM96_RST, GPIO_MODE_OUTPUT);
   gpio_pad_select_gpio(PIN_RFM96_SPI_CS);
   gpio_set_direction(PIN_RFM96_SPI_CS, GPIO_MODE_OUTPUT);
